...
vardTypeComp = {'FSA':u'i4','FIRA':u'i4','GRDFLX':u'i4','HFX':u'i4',\
                ...
                'LWDOWN':u'i4','Q2D':u'i4','PSFC':u'i4','RAINRATE':u'i4'}
...
def main(in_nc,filetype,out_path,filesList,errTitle,emailAddy,lockFile,initTime,validTime,\
         isGEOGRID=False, isGIS=False, isCompress=False):
    ...
    for wrf_file in filesList:
        ...
        if filetype == 'fe':
            ...
            # Add in model initialization and valid time to forcing files
            rootgrp2.TITLE = "OUTPUT FROM HRLDAS v20150506" 
            rootgrp2.model_initialization_time = initTime.strftime('%Y-%m-%d_%H') + ':00:00'
            rootgrp2.model_output_valid_time = validTime.strftime('%Y-%m-%d_%H') + ':00:00'
            ...
            if isCompress:
                if ncvar.dtype == 'int32':
                    ...
                    if vardTypeComp[varname] != 'f4':
                        # Special conversion for RAINRATE from mm/sec to mm/hour
                        if varname == 'RAINRATE':
                           varTmp[indValid] = varTmp[indValid]*3600.0
                           del var.units
                           var.units = 'mm hr^-1'
...
def compressNWM(fileIn,fileOut,nwmType,metaFile,errTitle,emailAddy,lockFile,initTime=None,validTime=None):
   files_list = []
